...
function checkComment(){
    var commentName = document.getElementById("input-name");
    const name = commentName.value;
    var goodData = true;

    if (typeof name != 'string' || name.length === 0){
        alert("You must enter a name for the comment to be submitted");
        goodData = false;
    }

    var commentBox = document.getElementById("comment-input-field");
    const comment = commentBox.value;

    if (typeof comment != 'string' || comment.length === 0){
        alert("You must enter a comment for it to be submitted");
        goodData = false;
    }
